...
turkey02 <- read_dta("Turkey-2002--full-data-5.dta")
turkey05 <- read_dta("Turkey-2005--full data-5.dta")
turkey08 <- read_dta("Turkey-2008--full-data-.dta")
turkey13 <- read_dta("Turkey-2013-full-data.dta")
turkey15 <- read_dta("Turkey-2015-full-data.dta")
turkey19 <- read_dta("Turkey-2019-full-data.dta")

ecpi <- read_dta("agg_ecpi.dta")
annual_inflation <- read_dta("annual.dta")

subset_turkey19 <- turkey19[c(1,2,3,5,11,12,13,16,26,27,28,29,32,33,34,35,42,
                              43,48,51,60,61,62,63,64,70,71,72,74,75,76,77,78,
                              79,80,81,82,105,120,121,122,128,167,189,301,310,
                              311,312,313,314,315,344,345,372,373,375,376,377,
                              378,381,382,383,384,385,386,387,388,389,392,393,
                              394,395,396,397,398,399,400,401,403,404,405,406,
                              407,408,409,410,411,412,413,423,424,425,426,427,
                              428,429)]
...
SC_turkey2008 <- turkey08 %>% select(a1, a4b, a14m, a14y, b1, b3, b2a, b2b, b2c,
                                     ECAq5, b5, c6, c7, c8, c9b, c11, c15, c16, 
                                     c17, c23, c24b, c24c, c24d, c25, c26, c27, 
                                     c30a, c30b, d1a2, d1a3, d2, d30a, e3, e4, 
                                     e16, e17, ECAq53, ECAo1, ECAo2, ECAo11, 
                                     ECAo12, ECAo14, ECAo15, f1,f2,k4, n5a, n5b,
                                     l1, l2, l3a, l3b, l4a, l4b, l10, l11a, l11b)
SC_turkey2008$country <- 'Turkey'

SC_turkey2013 <- turkey13 %>% select(a1, a4b, a14m, a14y, b1, b3, b2a, b2b, b2c,
                                     ECAq5, b5, c6, c7, c8, c9b, c11, c15, c16, 
                                     c17, c23, c30a, c30b, d1a2, d1a3, d2, d30a,
                                     ECAq53, f1, f2, k4, n5a, n5b, l1, l2, l3a, 
                                     l3b, l4a, l4b, l10, l11a, l11b)
SC_turkey2013$country <- 'Turkey'

SC_turkey2015 <- turkey15 %>% select(a1, a4b, a14m, a14y, tub1, b3, b2a, b2b, 
                                     b2c, b5, c6, c7, c8a, c8b, c9a, c11, c15, 
                                     c16, c17, tuc10, tuc11, tuc12, tuc13, d1a2, 
                                     d1a3, d2, f1,f2,k4, n5a, n5b, l1, l2, l5, 
                                     l6, l6a, l8)
SC_turkey2015$country <- 'Turkey'
names(SC_turkey2015)[names(SC_turkey2015) == 'tub1'] <- 'b1' 

SC_turkey2019 <- turkey19 %>% select(a1, a4b, a14m, a14y, b1, b3, b2a, b2b, b2c,
                                     b5, c6, c7, c8a, c8b, c9a, c9b, c11, c15,
                                     c16, c17, c30a, d1a2, d1a3, d2, d30a, f1,
                                     f2,k4, n5a, n5b, l1, l2, l3a, l3b)
...
inflation_monthly$yearmonth <- paste(inflation_monthly$year, 
                                     inflation_monthly$month)
...
trySC <- subset(total_4, f1 > 0)
summary(trySC$f1)
boxplot(trySC$f1)

OC <- ggplot(data=trySC, mapping=aes(x=f2, y=f1)) +
  geom_point(size=1) 
OC

OC <- ggplot(data=turkey2015, mapping=aes(x=logf2, y=logf1)) +
  geom_point(size=1)
OC

turkey2015$logf2 <- log(turkey2015$f2)
turkey2015$logf1 <- log(turkey2015$f1)

if (trySC$f1<60) print(trySC$f1)

------------x-------------x-----------------x-------------------x---------------x
library(gamlr)

y <- total_4$c7 + total_4$c8 + total_4$c16 + total_4$c17 + total_4$c26 + total_4$c27 +
  total_4$ECAo1 + total_4$ECAo11 + total_4$ECAo12 + total_4$ECAo15 + total_4$l11a

tryyy <- gamlr(total_4, y, 
               family=c("gaussian"))
